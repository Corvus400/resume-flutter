...
ポートフォリオリポジトリのため外部 PR は受け付けません。依存更新通知とセキュリティ通知のため Issue は有効化しています。
...
- **公開前 hardening** — MIT License、Security Policy、Dependabot、GitHub Actions SHA pinning、画像メタデータ除去を適用
...
- GitHub Actions と workflow の依存更新は手動レビューで適用します。
...
- tracked tree と Git 履歴に対して、秘密情報・ローカル絶対パス・個人メールの混入を確認します。
- README 用画像と Web アイコン類は、表示に必要な画像データを残したまま PNG メタデータを除去します。
